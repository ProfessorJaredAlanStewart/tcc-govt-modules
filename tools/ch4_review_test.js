...
const F="Chapter 4 - Civil Liberties (Expanded Edition).html";
...
setTimeout(()=>{
  ...
  const entries=[...d.querySelectorAll('[onclick]')].filter(e=>/goToSection\('review'\)/.test(e.getAttribute('onclick')||''));
  ...
  ck(w.eval('gameState.currentScore')===0,'review page changed the score (gameState.currentScore)');
