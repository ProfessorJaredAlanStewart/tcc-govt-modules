...
const F="Chapter 2 - The Constitution and Its Origins (Expanded Edition).html";
...
setTimeout(()=>{
  ...
  ck(d.getElementById('page-home').classList.contains('active'),'home not active on load');

  // the button exists and opens it
  const entries=[...d.querySelectorAll('[onclick]')].filter(e=>/showPage\('review'\)/.test(e.getAttribute('onclick')||''));
  ...
  ck(!d.getElementById('page-home').classList.contains('active'),'home still active after navigating');
  ...
  // back to menu works
  w.showPage('home');
  ck(d.getElementById('page-home').classList.contains('active'),'back to home failed');
  ...
  ck(d.querySelectorAll('#page-review details').length>=10,'expected 10 review questions');
